 ...
 */
public class CategoriaGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headersHttp).check(status().is(401)))
        ...
        .repeat(2)
        .on(
            exec(http("Get all categorias").get("/api/categorias").headers(headersHttpAuthenticated).check(status().is(200)))
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new categoria")
                        .post("/api/categorias")
                ...
                .pause(10)
                .repeat(5)
                .on(exec(http("Get created categoria").get("${new_categoria_url}").headers(headersHttpAuthenticated)).pause(10))
                .exec(http("Delete created categoria").delete("${new_categoria_url}").headers(headersHttpAuthenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the Categoria entity").exec(scn);
