 ...
 */
public class ProductoGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headersHttp).check(status().is(401)))
        ...
        .repeat(2)
        .on(
            exec(http("Get all productos").get("/api/productos").headers(headersHttpAuthenticated).check(status().is(200)))
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new producto")
                        .post("/api/productos")
                ...
                .pause(10)
                .repeat(5)
                .on(exec(http("Get created producto").get("${new_producto_url}").headers(headersHttpAuthenticated)).pause(10))
                .exec(http("Delete created producto").delete("${new_producto_url}").headers(headersHttpAuthenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the Producto entity").exec(scn);
